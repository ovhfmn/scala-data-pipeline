...
RUN useradd -r -u 1001 -g root banking_pipeline
RUN mkdir -p /app/logs /app/data && \
    chown -R banking_pipeline:root /app

USER banking_pipeline

COPY --from=builder /build/target/scala-2.13/pipeline-service.jar ./app.jar
ENTRYPOINT ["java", \
            "-XX:+UseG1GC", \
